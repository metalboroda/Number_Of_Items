...
namespace Assets.Scripts.Infrastructure
{
  public class SceneLoader
  {
    ...
    public void LoadSceneAsync(string sceneName, Action callback) {
      SceneManager.LoadSceneAsync(sceneName, LoadSceneMode.Single).completed += (asyncOp) => {
        callback?.Invoke();
      };
    }
    ...
    public void RestartSceneAsync(Action callback) {
      SceneManager.LoadSceneAsync(SceneManager.GetActiveScene().buildIndex, LoadSceneMode.Single).completed += (asyncOp) => {
        callback?.Invoke();
      };
    }
  }
